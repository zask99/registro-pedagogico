# mi_admin/management/commands/data.py
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        for i, materia_name in enumerate(materias_data, start=1):
            ...
            # If user already exists, ensure their email is correct and role is Docente
            else:
                if user.email != correo or user.rol != roles['Docente']:
                    user.email = correo
                    user.rol = roles['Docente']
                    user.save()
                    self.stdout.write(f"Usuario {user.username} actualizado.")


            # Crear o actualizar docente
            # Primero, intenta encontrar por CI, ya que es más probable que sea único
            try:
                ...
                docente.correo = correo # Asegúrate de actualizar el correo aquí también
                docente.save()
                self.stdout.write(f"Docente {docente.nombres} {docente.apellidos} actualizado.")
            except Docente.DoesNotExist:
                # Si no existe por CI, intenta encontrar por correo
                try:
                    docente = Docente.objects.get(correo=correo)
                    self.stdout.write(self.style.WARNING(
                        f"Docente con correo '{correo}' ya existe, pero con CI diferente. No se creó un nuevo docente para CI '{ci}'. Se actualizará el existente."
                    ))
                    # Si el correo existe, actualiza los datos del docente existente
                    docente.ci = ci # Considera si quieres actualizar el CI si el correo es el mismo
                    docente.nombres = nombres
                    docente.apellidos = apellidos
                    docente.expedido = expedido
                    docente.celular = celular
                    ...
                    docente.usuario = user
                    docente.save()
                    self.stdout.write(f"Docente {docente.nombres} {docente.apellidos} (correo: {correo}) actualizado.")
                except Docente.DoesNotExist:
                    # Si no existe ni por CI ni por correo, entonces crea uno nuevo
                    ...
                    self.stdout.write(f"Docente {docente.nombres} {docente.apellidos} creado.")
            docentes[materia_name] = docente # Asegura que el diccionario se actualice con el docente correcto
        ...
        for curso_name in cursos_primaria:
            ...
            for i in range(1, 6):  # 5 estudiantes
                ...
                if user_created:
                    ...
                    if user.email != correo or user.rol != roles['Estudiante']:
                        user.email = correo
                ...
                try:
                    ...
                    estudiante.correo = correo # Asegúrate de actualizar el correo aquí
                    estudiante.save()
                    self.stdout.write(f"Estudiante {estudiante.nombres} {estudiante.apellidos} actualizado.")
                except Estudiante.DoesNotExist:
                    # Si no existe por CI, intenta encontrar por correo
                    try:
                        estudiante = Estudiante.objects.get(correo=correo)
                        self.stdout.write(self.style.WARNING(
                            f"Estudiante con correo '{correo}' ya existe, pero con CI diferente. No se creó un nuevo estudiante para CI '{ci}'. Se actualizará el existente."
                        ))
                        # Si el correo existe, actualiza los datos del estudiante existente
                        estudiante.ci = ci # Considera si quieres actualizar el CI si el correo es el mismo
                        estudiante.nombres = nombres
                        estudiante.apellidos = apellidos
                        estudiante.expedido = expedido
                        estudiante.celular = celular
                        ...
                        estudiante.usuario = user
                        estudiante.save()
                        self.stdout.write(f"Estudiante {estudiante.nombres} {estudiante.apellidos} (correo: {correo}) actualizado.")
                    except Estudiante.DoesNotExist:
                        # Si no existe ni por CI ni por correo, entonces crea uno nuevo
                        ...
                        self.stdout.write(f"Estudiante {estudiante.nombres} {estudiante.apellidos} creado.")
